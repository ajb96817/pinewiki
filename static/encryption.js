...
function encryption_editFormOnSubmit() {
...
function encryptForSubmit() {
   ...
   if(!(wikitext=document.getElementById('edit_page_content'))) {
      alert("Could not locate page content.");
      encryptForSubmitInUse=false; return(false);
   }
   if(!(hiddentext=document.getElementById('encrypted_page_content'))) {
      alert("Could not locate encrypted text container.");
      encryptForSubmitInUse=false; return(false);
   }
...
function decryptEditForm() {
  var elem=null, newtext="";
    if(!(elem=document.getElementById('edit_page_content'))) {
	alert("Could not locate page content.");
    return(true);
  }
  if((newtext=decryptMixedText(elem.value))===false) {
    alert("Failed to decrypt page text");
    return(false);
  }
  elem.value=newtext;
  return(true);
...
*/
function toggleCryptDiv(elemid) {
   var elem=null, atab=null, key="", ptext="";
//   var ctStr="Decrypt Encrypted Text", ptStr="Hide Plaintext";
   elem=document.getElementById(elemid);
    if(elem===null) {
	alert("Could not find encrypted chunk: " + elemid);
	return;
    }
    ciphertext = elem.innerHTML;
    
    // decrypt text
    if((ptext=verifyDecrypt(ciphertext,'default',false))===false) {
        alert("Decryption failed.");
        return;
    }
    var escaped_ptext = ptext.replaceAll('<', '&lt;').replaceAll('>', '&gt;');
    elem.innerHTML='<pre>' + escaped_ptext + '</pre>';
    elem.parentNode.className = 'encrypted_chunk decrypted';
}
...
function getEncryptionKeyForLock(lock) {
  ...
  if(undefined===crypt_keys[lock]) {
    var x,y;
    x=prompt("Enter encryption password");
    if(x===null) { return(false); }
    y=prompt("Verify password");
...
function encryptMixedText(x) {
  ...
  while((cur=x.indexOf("<" + tag,pos))!=-1) {
    ...
    if(!(ctext=encryptBlock(x.substring(cur,closetag+tag.length+3),false))) {
      alert("Failed to encrypt text.  Please double check your encryption password.");
      return(false);
    }
    ret+=x.substring(pos,cur) + ctext;
    pos=closetag+tag.length+3;
...
function verifyDecrypt(ctext,lock,key) {
  ...
  if(key===false && (undefined===crypt_keys[lock])) {
    var key=prompt("Enter encryption password");
    if(key===null) { return(false); } // user hit cancel
    if(!(ptext=decryptTextString(ctext,key))) {
	var pstr="Try again: Enter encryption password";
